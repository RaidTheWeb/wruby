...
function integrateWasmJS() {
  ...
  var wasmTextFile = 'wruby.wast';
  var wasmBinaryFile = 'wruby.wasm';
  var asmjsCodeFile = 'wruby.temp.asm.js';
...
// {{PRE_LIBRARY}}
        ...
        FS.registerDevice(dev, TTY.stream_ops);
          ...
          if (!tty.input.length) {
            ...
            if (ENVIRONMENT_IS_NODE) {
              ...
              // Browser.
              result = nextInput();//window.prompt('Input: ');  // returns null on cancel
              if (result !== null) {
                result += '\n';
              }
  ...
  var __sigalrm_handler=0;function _signal(sig, func) {
      if (sig == 14 /*SIGALRM*/) {
        ...
        // backtrace()
        err('Calling stub instead of signal()',sig,func);
      }
      return 0;
    }
...
evaluatus=function (result){
  console.log("Module.callMain")
  Module.callMain()
            // web_tty.input = intArrayFromString(result, true);
