...
source_file_new(_state *mrb, _debug_context *dbg, char *filename)
{
  ...
  file = (source_file*)_malloc(mrb, sizeof(source_file));
  ...
  file->lineno = 1;
  file->path = (char*)_malloc(mrb, strlen(filename) + 1);
...
_debug_get_source(_state *mrb, mrdb_state *mrdb, const char *srcpath, const char *filename)
{
  ...
  search_path[1] = dirname(mrb, _debug_get_filename(mrdb->dbg->irep, 0));
  search_path[2] = ".";
