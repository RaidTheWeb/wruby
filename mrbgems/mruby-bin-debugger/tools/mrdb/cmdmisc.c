...
parse_filename(state *mrb, char **sp, listcmd_parser_state *st)
{
  ...
  if (len > 0) {
    st->filename = (char*)_malloc(mrb, len + 1);
...
replace_ext(state *mrb, const char *filename, const char *ext)
{
  ...
  s = (char*)_malloc(mrb, len + strlen(ext) + 1);
...
dbgcmd_list(state *mrb, mrdb_state *mrdb)
{
  ...
  if (parse_listcmd_args(mrb, mrdb, st)) {
    if ((filename = _debug_get_source(mrb, mrdb, mrdb->srcpath, st->filename)) == NULL) {
      filename = st->filename;
    }
    _debug_list(mrb, mrdb->dbg, filename, st->line_min, st->line_max);
...
dbgcmd_quit(state *mrb, mrdb_state *mrdb)
{
  ...
  if (mrdb->dbg->xm == DBG_QUIT) {
    struct RClass *exc;
    exc = _define_class(mrb, "DebuggerExit", _class_get(mrb, "Exception"));
    _raise(mrb, exc, "Exit mrdb.");
  }
  return DBGST_PROMPT;
}
