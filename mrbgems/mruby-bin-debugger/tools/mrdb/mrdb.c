...
static int32_t
check_method_breakpoint(state *mrb, _irep *irep, _code *pc, value *regs)
{
  struct RClass* c;
  _sym sym;
  int32_t bpno;
...
static void
_code_fetch_hook(state *mrb, _irep *irep, _code *pc, value *regs)
{
  const char *file;
  int32_t line;
  int32_t bpno;
...
static mrdb_exemode
_debug_break_hook(state *mrb, _debug_context *dbg)
{
  debug_command *cmd;
