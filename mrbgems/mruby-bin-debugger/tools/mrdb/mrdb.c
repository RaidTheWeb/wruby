...
void mrdb_state_free($state *);

static $debug_context *_debug_context = NULL;
...
append_srcpath:
        if (!args->srcpath) {
          ...
          itemlen = strlen(item);
          args->srcpath =
            (char *)$realloc(mrb, args->srcpath, srcpathlen + itemlen + 2);
...
static $debug_context*
$debug_context_new($state *mrb)
{
  $debug_context *dbg = ($debug_context*)$malloc(mrb, sizeof($debug_context));

  memset(dbg, 0, sizeof($debug_context));
...
$debug_context*
$debug_context_get($state *mrb)
{
  if (!_debug_context) {
    _debug_context = $debug_context_new(mrb);
  }
  return _debug_context;
}

void
$debug_context_set($debug_context *dbg)
{
  _debug_context = dbg;
}

void
$debug_context_free($state *mrb)
{
  if (_debug_context) {
    $debug_delete_break_all(mrb, _debug_context);
    $free(mrb, _debug_context);
    _debug_context = NULL;
  }
}

static mrdb_state*
mrdb_state_new($state *mrb)
{
  mrdb_state *mrdb = (mrdb_state*)$malloc(mrb, sizeof(mrdb_state));

  memset(mrdb, 0, sizeof(mrdb_state));

  mrdb->dbg = $debug_context_get(mrb);
  mrdb->command = (char*)$malloc(mrb, MAX_COMMAND_LINE+1);
  mrdb->print_no = 1;
...
void
mrdb_state_free($state *mrb)
{
  $debug_context_free(mrb);
  if (_mrdb_state) {
    $free(mrb, _mrdb_state->command);
    $free(mrb, _mrdb_state);
    _mrdb_state = NULL;
  }
}

static char*
get_command($state *mrb, mrdb_state *mrdb)
{
  int i;
  int c;
...
static char*
pick_out_word($state *mrb, char **pp)
{
  char *ps;
...
parse_command($state *mrb, mrdb_state *mrdb, char *buf)
{
...
static void
print_info_stopped_break($state *mrb, mrdb_state *mrdb)
{
  $debug_breakpoint bp;
  int32_t ret;
  uint16_t lineno;
  ...
  ret = $debug_get_break(mrb, mrdb->dbg, mrdb->dbg->stopped_bpno, &bp);
  if (ret == 0) {
    switch(bp.type) {
...
static void
print_info_stopped_step_next($state *mrb, mrdb_state *mrdb)
{
...
static void
print_info_stopped_code($state *mrb, mrdb_state *mrdb)
{
  char* file = $debug_get_source(mrb, mrdb, mrdb->srcpath, mrdb->dbg->prvfile);
  uint16_t lineno = mrdb->dbg->prvline;
  if (file != NULL) {
    $debug_list(mrb, mrdb->dbg, file, lineno, lineno);
    $free(mrb, file);
  }
}

static void
print_info_stopped($state *mrb, mrdb_state *mrdb)
{
  switch(mrdb->dbg->bm) {
    case BRK_BREAK:
...
static debug_command*
get_and_parse_command($state *mrb, mrdb_state *mrdb)
{
  debug_command *cmd = NULL;
  char *p;
  int i;
...
static int32_t
check_method_breakpoint($state *mrb, $irep *irep, $code *pc, $value *regs)
{
  struct RClass* c;
  $sym sym;
  int32_t bpno;
  $bool isCfunc;
  struct $insn_data insn;

  $debug_context *dbg = $debug_context_get(mrb);
  ...
  if (sym != 0) {
    dbg->method_bpno = $debug_check_breakpoint_method(mrb, dbg, c, sym, &isCfunc);
    if (isCfunc) {
      bpno = dbg->method_bpno;
      dbg->method_bpno = 0;
    }
...
static void
$code_fetch_hook($state *mrb, $irep *irep, $code *pc, $value *regs)
{
  const char *file;
  int32_t line;
  int32_t bpno;
  ...
  file = $debug_get_filename(irep, pc - irep->iseq);
  line = $debug_get_line(irep, pc - irep->iseq);
  ...
  case DBG_RUN:
    ...
    if (dbg->prvfile != file || dbg->prvline != line) {
      bpno = $debug_check_breakpoint_line(mrb, dbg, file, line);
      if (bpno > 0) {
        dbg->stopped_bpno = bpno;
        dbg->bm = BRK_BREAK;
        break;
...
static mrdb_exemode
$debug_break_hook($state *mrb, $debug_context *dbg)
{
  debug_command *cmd;
...
main(int argc, char **argv)
{
 ...
 l_restart:
  ...
  mrdb->dbg->break_hook = $debug_break_hook;

  if (args.mrbfile) { /* .mrb */
    v = $load_irep_file(mrb, args.rfp);
  }
  else {              /* .rb */
    mrbc_context *cc = mrbc_context_new(mrb);
    mrbc_filename(mrb, cc, args.fname);
    v = $load_file_cxt(mrb, args.rfp, cc);
    mrbc_context_free(mrb, cc);
  }
  if (mrdb->dbg->xm == DBG_QUIT && !$undef_p(v) && mrb->exc) {
    const char *classname = $obj_classname(mrb, $obj_value(mrb->exc));
    ...
    if (!strcmp(classname, "DebuggerRestart")) {
      ...
      dbg_backup = $debug_context_get(mrb);

      mrdb_state_set(NULL);
      $debug_context_set(NULL);

      cleanup(mrb, &args);
      mrb = $open();

      mrdb_state_set(mrdb_backup);
      $debug_context_set(dbg_backup);
