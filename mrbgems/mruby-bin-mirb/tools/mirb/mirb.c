...
get_history_path(_state *mrb)
{
  ...
  if (home != NULL) {
    ...
    if (len >= 0) {
      size_t size = len + 1;
      path = (char *)_malloc_simple(mrb, size);
      if (path != NULL) {
...
p(_state *mrb, _value obj, int prompt)
{
  ...
  if (prompt) {
    if (!mrb->exc) {
      ...
      val = _funcall(mrb, _obj_value(mrb->exc), "inspect", 0);
    }
  }
  if (!_string_p(val)) {
    val = _obj_as_string(mrb, obj);
  }
  msg = _locale_from_utf8(RSTRING_PTR(val), (int)RSTRING_LEN(val));
...
static _bool
is_code_block_open(struct _parser_state *parser)
{
...
main(int argc, char **argv)
{
  ...
  while (TRUE) {
    ...
    utf8 = _utf8_from_locale(ruby_code, -1);
    if (!utf8) abort();

    /* parse code */
    parser = _parser_new(mrb);
    if (parser == NULL) {
    ...
    if (code_block_open) {
      ...
      if (0 < parser->nwarn) {
        /* warning */
        char* msg = _locale_from_utf8(parser->warn_buffer[0].message, -1);
        printf("line %d: %s\n", parser->warn_buffer[0].lineno, msg);
        _locale_free(msg);
      }
      if (0 < parser->nerr) {
        /* syntax error */
        char* msg = _locale_from_utf8(parser->error_buffer[0].message, -1);
        printf("line %d: %s\n", parser->error_buffer[0].lineno, msg);
        _locale_free(msg);
      }
      else {
        /* generate bytecode */
        struct RProc *proc = _generate_code(mrb, parser);
        if (proc == NULL) {
          fputs("codegen error\n", stderr);
          _parser_free(parser);
          break;
        ...
        if (mrb->exc) {
          p(mrb, _obj_value(mrb->exc), 0);
          mrb->exc = 0;
        }
        else {
          /* no */
          if (!_respond_to(mrb, result, _intern_lit(mrb, "inspect"))){
            result = _any_to_s(mrb, result);
          }
          p(mrb, result, 1);
        }
      }
