...
#ifndef $CODEGEN_LEVEL_MAX
#define $CODEGEN_LEVEL_MAX 1024
#endif
...
{
  void *p = $malloc_simple(s->mrb, len);

  if (!p) codegen_error(s, "$malloc");
  return p;
}
...
{
  p = $realloc_simple(s->mrb, p, len);

  if (!p && len > 0) codegen_error(s, "$realloc");
  return p;
}
...
emit_B(codegen_scope *s, uint32_t pc, uint8_t i)
{
  ...
  if (pc >= s->icapa) {
    ...
    s->iseq = ($code *)codegen_realloc(s, s->iseq, sizeof($code)*s->icapa);
    if (s->lines) {
...
static void
genop_0(codegen_scope *s, $code i)
{
  s->lastpc = s->pc;
  gen_B(s, i);
}
...
static void
genop_1(codegen_scope *s, $code i, uint16_t a)
{
  s->lastpc = s->pc;
  if (a > 0xff) {
    gen_B(s, OP_EXT1);
...
static void
genop_2(codegen_scope *s, $code i, uint16_t a, uint16_t b)
{
  s->lastpc = s->pc;
  if (a > 0xff && b > 0xff) {
    gen_B(s, OP_EXT3);
...
static void
genop_3(codegen_scope *s, $code i, uint16_t a, uint16_t b, uint8_t c)
{
  genop_2(s, i, a, b);
  gen_B(s, c);
}
...
static void
genop_2S(codegen_scope *s, $code i, uint16_t a, uint16_t b)
{
  genop_1(s, i, a);
  gen_S(s, b);
}
...
static void
genop_W(codegen_scope *s, $code i, uint32_t a)
{
...
static struct $insn_data
$last_insn(codegen_scope *s)
{
  if (s->pc == s->lastpc) {
  ...
  return $decode_insn(&s->iseq[s->lastpc]);
}
...
static uint16_t
genjmp(codegen_scope *s, $code i, uint16_t pc)
{
  uint16_t pos;
...
static uint16_t
genjmp2(codegen_scope *s, $code i, uint16_t a, int pc, int val)
{
  uint16_t pos;

  if (!no_peephole(s) && !val) {
    struct $insn_data data = $last_insn(s);
...
new_lit(codegen_scope *s, $value val)
{
  ...
  case $TT_FLOAT:
    for (i=0; i<s->irep->plen; i++) {
      pv = &s->irep->pool[i];
      if ($type(*pv) != $TT_FLOAT) continue;
      if ($float(*pv) == $float(val)) return i;
    }
    break;
#endif
  case $TT_FIXNUM:
    for (i=0; i<s->irep->plen; i++) {
      pv = &s->irep->pool[i];
      if (!$fixnum_p(*pv)) continue;
      if ($fixnum(*pv) == $fixnum(val)) return i;
    }
    break;
  default:
    /* should not happen */
  ...
  switch ($type(val)) {
  case $TT_STRING:
    *pv = $str_pool(s->mrb, val);
    break;

#ifndef $WITHOUT_FLOAT
  case $TT_FLOAT:
#ifdef $WORD_BOXING
    *pv = $float_pool(s->mrb, $float(val));
    break;
#endif
#endif
  case $TT_FIXNUM:
...
static int
new_sym(codegen_scope *s, $sym sym)
{
  int i, len;
  ...
  if (s->irep->slen >= s->scapa) {
    s->scapa *= 2;
    s->irep->syms = ($sym*)codegen_realloc(s, s->irep->syms, sizeof($sym)*s->scapa);
  }
...
#define nsym(x) (($sym)(intptr_t)(x))

#define lv_name(lv) nsym((lv)->car)

static int
lv_idx(codegen_scope *s, $sym id)
{
  node *lv = s->lv;
  int n = 1;
...
for_body(codegen_scope *s, node *tree)
{
  ...
  pop();
  idx = new_sym(s, $intern_lit(s->mrb, "each"));
...
lambda_body(codegen_scope *s, node *tree, int blk)
{
  ...
  if (tree->car) {
    ...
    if (tail) {
      ...
      $assert(nint(tail->car) == NODE_ARGS_TAIL);
      $assert(node_len(tail) == 4);

      while (kwds) {
        int jmpif_key_p, jmp_def_set = -1;
        node *kwd = kwds->car, *def_arg = kwd->cdr->cdr->car;
        $sym kwd_sym = nsym(kwd->cdr->car);

        $assert(nint(kwd->car) == NODE_KW_ARG);
...
static $sym
attrsym(codegen_scope *s, $sym a)
{
  const char *name;
  $int len;
  char *name2;

  name = $sym2name_len(s->mrb, a, &len);
  ...
  return $intern(s->mrb, name2, len+1);
}
...
static void
gen_call(codegen_scope *s, node *tree, $sym name, int sp, int val, int safe)
{
  $sym sym = name ? name : nsym(tree->cdr->car);
  int skip = 0;
  ...
  {
    $int symlen;
    const char *symname = $sym2name_len(s->mrb, sym, &symlen);
...
static void
gen_literal_array(codegen_scope *s, node *tree, $bool sym, int val)
{
  if (val) {
    int i = 0, j = 0;
...
{
  int idx = new_lit(s, $str_new_cstr(s->mrb, msg));
...
static $int
readint_$int(codegen_scope *s, const char *p, int base, $bool neg, $bool *overflow)
{
  const char *e = p + strlen(p);
  $int result = 0;
  int n;
...
codegen(codegen_scope *s, node *tree, int val)
{
  ...
  s->rlev++;
  if (s->rlev > $CODEGEN_LEVEL_MAX) {
    codegen_error(s, "too complex expression");
  }
  if (s->irep && s->filename_index != tree->filename_index) {
    s->irep->filename = $parser_get_filename(s->parser, s->filename_index);
    $debug_info_append_file(s->mrb, s->irep, s->debug_start_pos, s->pc);
    s->debug_start_pos = s->pc;
    s->filename_index = tree->filename_index;
    s->filename = $parser_get_filename(s->parser, tree->filename_index);
  }
        ...
        while (n2) {
          ...
          do {
            if (n4 && n4->car && nint(n4->car->car) == NODE_SPLAT) {
              ...
              pop();
              genop_3(s, OP_SEND, cursp(), new_sym(s, $intern_lit(s->mrb, "__case_eqq")), 1);
            }
            else {
              if (n4) {
                codegen(s, n4->car, VAL);
                ...
                genop_2(s, OP_GETCONST, cursp(), new_sym(s, $intern_lit(s->mrb, "StandardError")));
                push();
              }
              pop();
        ...
        while (n) {
          ...
          if (head) {
            ...
            if (nint(n->car->car) == NODE_SPLAT) {
              ...
              genop_3(s, OP_SEND, cursp(), new_sym(s, $intern_lit(s->mrb, "===")), 1);
            }
          }
          else {
            pop();
  ...
  case NODE_OP_ASGN:
    {
      $sym sym = nsym(tree->cdr->car);
      $int len;
      const char *name = $sym2name_len(s->mrb, sym, &len);
  ...
  case NODE_YIELD:
    {
      ...
      if (sendv) n = CALL_MAXARGS;
      genop_3(s, OP_SEND, cursp(), new_sym(s, $intern_lit(s->mrb, "call")), n);
      if (val) push();
    }
    break;
  ...
  case NODE_BACK_REF:
    if (val) {
      ...
      buf[1] = nchar(tree);
      buf[2] = 0;
      sym = new_sym(s, $intern_cstr(s->mrb, buf));
      genop_2(s, OP_GETGV, cursp(), sym);
      push();
    }
    break;
  ...
  case NODE_NTH_REF:
    if (val) {
      ...
      str = $format(mrb, "$%S", $fixnum_value(nint(tree)));
      sym = new_sym(s, $intern_str(mrb, str));
      genop_2(s, OP_GETGV, cursp(), sym);
      push();
    }
    break;
  ...
  case NODE_INT:
    if (val) {
      ...
      i = readint_$int(s, p, base, FALSE, &overflow);
#ifndef $WITHOUT_FLOAT
      if (overflow) {
        double f = readint_float(s, p, base);
        int off = new_lit(s, $float_value(s->mrb, f));
...
#endif
      {
        if (i == -1) genop_1(s, OP_LOADI__1, cursp());
          ...
          int off = new_lit(s, $fixnum_value(i));
          genop_2(s, OP_LOADL, cursp(), off);
        }
      }
      push();
  ...
  case NODE_NEGATE:
    {
      ...
      case NODE_INT:
        if (val) {
          ...
          i = readint_$int(s, p, base, TRUE, &overflow);
#ifndef $WITHOUT_FLOAT
          if (overflow) {
            double f = readint_float(s, p, base);
            int off = new_lit(s, $float_value(s->mrb, -f));
      ...
      default:
        if (val) {
          int sym = new_sym(s, $intern_lit(s->mrb, "-@"));
          codegen(s, tree, VAL);
          pop();
  ...
  case NODE_STR:
    if (val) {
      ...
      int ai = $gc_arena_save(s->mrb);
      int off = new_lit(s, $str_new(s->mrb, p, len));

      $gc_arena_restore(s->mrb, ai);
  ...
  case NODE_HEREDOC:
    tree = ((struct $parser_heredoc_info *)tree)->doc;
    /* fall through */
  case NODE_DSTR:
    if (val) {
      node *n = tree;
  ...
  case NODE_DXSTR:
    {
      node *n;
      int ai = $gc_arena_save(s->mrb);
      int sym = new_sym(s, $intern_lit(s->mrb, "Kernel"));
      ...
      while (n) {
        if (nint(n->car->car) == NODE_XSTR) {
          n->car->car = (struct $ast_node*)(intptr_t)NODE_STR;
          $assert(!n->cdr); /* must be the end */
        }
        codegen(s, n->car, VAL);
        pop(); pop();
        genop_1(s, OP_STRCAT, cursp());
      ...
      push();                   /* for block */
      pop_n(3);
      sym = new_sym(s, $intern_lit(s->mrb, "`"));
  ...
  case NODE_REGX:
    if (val) {
      ...
      int ai = $gc_arena_save(s->mrb);
      int sym = new_sym(s, $intern_lit(s->mrb, REGEXP_CLASS));
      int off = new_lit(s, $str_new_cstr(s->mrb, p1));
      int argc = 1;
      ...
      if (p2 || p3) {
        if (p2) { /* opt */
          off = new_lit(s, $str_new_cstr(s->mrb, p2));
          genop_2(s, OP_STRING, cursp(), off);
        }
        else {
          genop_1(s, OP_LOADNIL, cursp());
        ...
        argc++;
        if (p3) { /* enc */
          off = new_lit(s, $str_new(s->mrb, p3, 1));
          genop_2(s, OP_STRING, cursp(), off);
          push();
          argc++;
        }
      ...
      pop_n(argc+2);
      sym = new_sym(s, $intern_lit(s->mrb, "compile"));
  ...
  case NODE_DREGX:
    if (val) {
      ...
      if (n->car) { /* tail */
        p = (char*)n->car;
        off = new_lit(s, $str_new_cstr(s->mrb, p));
        codegen(s, tree->car, VAL);
        genop_2(s, OP_STRING, cursp(), off);
        pop();
        genop_1(s, OP_STRCAT, cursp());
...
static void
scope_add_irep(codegen_scope *s, $irep *irep)
{
  if (s->irep == NULL) {
    s->irep = irep;
    return;
  ...
  if (s->irep->rlen == s->rcapa) {
    s->rcapa *= 2;
    s->irep->reps = ($irep**)codegen_realloc(s, s->irep->reps, sizeof($irep*)*s->rcapa);
  }
...
scope_new($state *mrb, codegen_scope *prev, node *lv)
{
  ...
  $pool *pool = $pool_open(mrb);
  codegen_scope *p = (codegen_scope *)$pool_alloc(pool, sizeof(codegen_scope));
  ...
  *p = codegen_scope_zero;
  ...
  p->rcapa = 8;
  p->irep->reps = ($irep**)$malloc(mrb, sizeof($irep*)*p->rcapa);

  p->icapa = 1024;
  p->iseq = ($code*)$malloc(mrb, sizeof($code)*p->icapa);
  p->irep->iseq = NULL;

  p->pcapa = 32;
  p->irep->pool = ($value*)$malloc(mrb, sizeof($value)*p->pcapa);
  p->irep->plen = 0;

  p->scapa = 256;
  p->irep->syms = ($sym*)$malloc(mrb, sizeof($sym)*p->scapa);
  p->irep->slen = 0;
  ...
  if (lv) {
    ...
    p->irep->lv = (struct $locals*)$malloc(mrb, sizeof(struct $locals) * (p->nlocals - 1));
    ...
    $assert(i + 1 == p->nlocals);
  }
  p->ai = $gc_arena_save(mrb);

  p->filename = prev->filename;
  if (p->filename) {
    p->lines = (uint16_t*)$malloc(mrb, sizeof(short)*p->icapa);
  }
...
scope_finish(codegen_scope *s)
{
  ...
  irep->flags = 0;
  if (s->iseq) {
    irep->iseq = ($code *)codegen_realloc(s, s->iseq, sizeof($code)*s->pc);
    irep->ilen = s->pc;
    if (s->lines) {
  ...
  irep->syms = ($sym*)codegen_realloc(s, irep->syms, sizeof($sym)*irep->slen);
  irep->reps = ($irep**)codegen_realloc(s, irep->reps, sizeof($irep*)*irep->rlen);
  if (s->filename) {
    irep->filename = $parser_get_filename(s->parser, s->filename_index);
    $debug_info_append_file(mrb, irep, s->debug_start_pos, s->pc);
...
generate_code($state *mrb, parser_state *p, int val)
{
  ...
  $TRY(&scope->jmp) {
    ...
    proc = $proc_new(mrb, scope->irep);
    $irep_decref(mrb, scope->irep);
    $pool_close(scope->mpool);
    proc->c = NULL;
