MRuby::Gem::Specification.new('mruby-test') do |spec|
  ...
  build.gems.each do |g|
    ...
    file g.test_rbireps => [g.test_rbfiles, build.mrbcfile].flatten do |t|
      ...
      open(t.name, 'w') do |f|
        ...
        f.puts %Q[void _#{g.funcname}_gem_test(state *mrb);] unless g.test_objs.empty?
        dep_list.each do |d|
          f.puts %Q[void GENERATED_TMP__#{d.funcname}_gem_init(state *mrb);]
          f.puts %Q[void GENERATED_TMP__#{d.funcname}_gem_final(state *mrb);]
        end
        f.puts %Q[void _init_test_driver(state *mrb, _bool verbose);]
        f.puts %Q[void _t_pass_result(state *dst, state *src);]
        f.puts %Q[void GENERATED_TMP__#{g.funcname}_gem_test(state *mrb) {]
        unless g.test_rbfiles.empty?
          ...
          g.test_rbfiles.count.times do |i|
            f.puts %Q[  ai = _gc_arena_save(mrb);]
            f.puts %Q[  mrb2 = _open_core(_default_allocf, NULL);]
            f.puts %Q[  if (mrb2 == NULL) {]
            f.puts %Q[    fprintf(stderr, "Invalid state, exiting \%s", __FUNCTION__);]
            f.puts %Q[    exit(EXIT_FAILURE);]
            f.puts %Q[  }]
            dep_list.each do |d|
              f.puts %Q[  GENERATED_TMP__#{d.funcname}_gem_init(mrb2);]
              f.puts %Q[  _state_atexit(mrb2, GENERATED_TMP__#{d.funcname}_gem_final);]
            end
            f.puts %Q[  _init_test_driver(mrb2, _test(_gv_get(mrb, _intern_lit(mrb, "$mrbtest_verbose"))));]
            if test_preload.nil?
              f.puts %Q[  _load_irep(mrb2, mrbtest_assert_irep);]
            else
              f.puts %Q[  _load_irep(mrb2, gem_test_irep_#{g.funcname}_preload);]
            end
            f.puts %Q[  if (mrb2->exc) {]
            f.puts %Q[    _print_error(mrb2);]
            f.puts %Q[    _close(mrb2);]
            f.puts %Q[    exit(EXIT_FAILURE);]
            f.puts %Q[  }]
            f.puts %Q[  _const_set(mrb2, _obj_value(mrb2->object_class), _intern_lit(mrb2, "GEMNAME"), _str_new(mrb2, "#{g.name}", #{g.name.length}));]

            unless g.test_args.empty?
              f.puts %Q[  test_args_hash = _hash_new_capa(mrb, #{g.test_args.length}); ]
              g.test_args.each do |arg_name, arg_value|
                escaped_arg_name = arg_name.gsub('\\', '\\\\\\\\').gsub('"', '\"')
                escaped_arg_value = arg_value.gsub('\\', '\\\\\\\\').gsub('"', '\"')
                f.puts %Q[  _hash_set(mrb2, test_args_hash, _str_new(mrb2, "#{escaped_arg_name.to_s}", #{escaped_arg_name.to_s.length}), _str_new(mrb2, "#{escaped_arg_value.to_s}", #{escaped_arg_value.to_s.length})); ]
              end
              f.puts %Q[  _const_set(mrb2, _obj_value(mrb2->object_class), _intern_lit(mrb2, "TEST_ARGS"), test_args_hash); ]
            end

            f.puts %Q[  _#{g.funcname}_gem_test(mrb2);] if g.custom_test_init?

            f.puts %Q[  _load_irep(mrb2, gem_test_irep_#{g.funcname}_#{i});]
            f.puts %Q[  ]

            f.puts %Q[  _t_pass_result(mrb, mrb2);]
            f.puts %Q[  _close(mrb2);]
  ...
  file clib => [init, build.mrbcfile, __FILE__] do |_t|
    ...
    open(clib, 'w') do |f|
      ...
      build.gems.each do |g|
        f.puts %Q[    GENERATED_TMP__#{g.funcname}_gem_test(mrb);]
      end
      f.puts %Q[}]
    end
  end
