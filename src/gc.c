...
#ifdef GC_DEBUG
void
test_field_write_barrier(void)
{
  state *mrb = open();
  struct RBasic *obj, *value;
  gc *gc = &mrb->gc;

  puts("test_field_write_barrier");
  gc->generational = FALSE;
  ...
  {
    puts("test_field_write_barrier_value");
    obj = basic_ptr(ary_new(mrb));
...
void
test_write_barrier(void)
{
  state *mrb = open();
  struct RBasic *obj;
  gc *gc = &mrb->gc;

  puts("test_write_barrier");
  obj = basic_ptr(ary_new(mrb));
  paint_black(obj);
